...
check_dockerapp_version_cmd=$(cat $docker_compose_file_url |grep APP_COMMON_DOCKER_VERSION |cut -d: -f2)


# check dockerapp version
sudo echo "{{docker_appname}}_version: `eval $check_dockerapp_version_cmd` |sudo tee -a /data/logs/install_version.txt
